...
class PrnBuilder {
	...
	public function __construct($base) {
		$list = array(0 => 'black', 1 => 'magenta', 2 => 'cyan', 4 => 'yellow', 17 => 'light_magenta', 18 => 'light_cyan');

		if (is_array($base)) {
			$list = array_flip($list);
			foreach($base as $plane_name => $img) {
				if (!isset($list[$plane_name])) throw new Exception('NO');
				if (is_resource($img)) {
					$this->planes[$list[$plane_name]] = $img;
				} else {
					$this->planes[$list[$plane_name]] = imagecreatefrompng($img);
				}
			}
